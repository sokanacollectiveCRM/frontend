...
export function UsersTable(_a) {
    ...
    return (_jsxs("div", { className: 'space-y-4', children: [_jsx("div", { className: 'rounded-md border', children: _jsxs(Table, { children: [_jsx(TableHeader, { children: table.getHeaderGroups().map(function (headerGroup) { return (_jsx(TableRow, { className: 'group/row', children: headerGroup.headers.map(function (header) {
                                    var _a, _b;
                                ...
                                }) }, headerGroup.id)); }) }), _jsx(TableBody, { children: ((_b = table.getRowModel().rows) === null || _b === void 0 ? void 0 : _b.length) ? (table.getRowModel().rows.map(function (row) { return (_jsx(TableRow, { "data-state": row.getIsSelected() && 'selected', className: 'group/row', children: row.getVisibleCells().map(function (cell) {
                                    var _a, _b;
