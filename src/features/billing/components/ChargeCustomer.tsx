...
export default function ChargeCustomer() {
  ...
  const handleCharge = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Successfully charged $${amount} to customer's default payment method`
      );
  ...
  const isSelectedCustomerInFilteredList = filteredCustomers.some(
  ...
  if (!user || user.role !== 'admin') {
    return (
      <Alert variant='destructive'>
        ...
        <AlertDescription>
          You do not have permission to access this feature. Admin access
          required.
        </AlertDescription>
      </Alert>
    );
